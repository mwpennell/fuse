...
p.con3 <- m2$par[argnames(lik.con3)]
m3 <- find.mle(lik.con3, p.con3)

lik.con3a <- constrain(lik.con2, q43~0)
p.con3a <- m2$par[argnames(lik.con3a)]
m3a <- find.mle(lik.con3a, p.con3a)

lik.con3b <- constrain(lik.con2, q21~0)
p.con3b <- m2$par[argnames(lik.con3b)]
m3b <- find.mle(lik.con3b, p.con3b)

## 4. Are XY -> XXY different from ZW -> ZWW
lik.con4 <- constrain(lik.con2, q34~q12)
p.con4 <- p[argnames(lik.con4)]
m4 <- find.mle(lik.con4, p.con4)
m4$lnLik
p.con4 <-m3$par[argnames(lik.con4)]
m4 <- find.mle(lik.con4, p.con4)
m4$lnLik

prior <- make.prior.exponential(20)

tmp <- mcmc(lik.con2, x.init = p.con2, w=1, prior=prior,
            nsteps=100, print.every=0)

w <- diff(sapply(tmp[2:7], range))

samp <- mcmc(lik.con2, x.init = p.con2, w=w, prior=prior,
             nsteps=50000, print.every=1000)

saveRDS(samp, "output/results/squa-final.rds")

rr <- sapply(seq(0.005, 5, by=0.005), function(x){
    tmp <- c(m2$par[1:5], x)
    names(tmp) <- argnames(lik.con2)
    lik.con2(tmp)})
