...
td <- readRDS("output/data/fish-indchrom.rds")
...
tmp <- mcmc(lik.thr, x.init = p.thr, w=1, prior=prior,
            nsteps=100, print.every=0)

w <- diff(sapply(tmp[2:7], range))

samp <- mcmc(lik.thr, x.init = p.thr, w=w, prior=prior,
             nsteps=50000, print.every=1000)

saveRDS(samp, "output/results/fish-indchrom.rds")
